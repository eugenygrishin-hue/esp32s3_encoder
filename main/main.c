...
static void knob_event_cb(void *arg, void *usr_data)
{
    ...
    // Получаем сырое значение из аппаратного счетчика
    last_raw_position = iot_knob_get_count_value(knob_handle);
    
    // Вычисляем реальную позицию с учетом нашего программного сброса
...
void app_main(void)
{
    ...
    if (btn_handle != NULL) {
        // Так как мы используем глобальные переменные, передавать knob_handle внутрь больше не нужно
